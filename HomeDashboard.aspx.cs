    ...
    public partial class HomeDashboard : System.Web.UI.Page
    {
        ...
        protected void LoadDataBypass(string cabang = "0")
        {
            ...
            if (cabang != "0" && cabang != "")
            {
                ...
                if (stsCabang == "1")
                {
                    mySet = ObjDb.GetRows("select p.namaCabang namaPerwakilan, a.namaCabang, a.linkUrl from mCabang a join mCabang p on p.noCabang = a.parent where a.stsCabang = '2' and p.noCabang = '" + cabang + "' order by p.namaCabang, a.namaCabang");
                }
                else
                {
                    mySet = ObjDb.GetRows("select p.namaCabang namaPerwakilan, a.namaCabang, a.linkUrl from mCabang a join mCabang p on p.noCabang = a.parent where a.stsCabang = '2' and a.noCabang = '" + cabang + "'  order by p.namaCabang, a.namaCabang");
                }
            }
            else
            {
                mySet = ObjDb.GetRows("select p.namaCabang namaPerwakilan, a.namaCabang, a.linkUrl from mCabang a join mCabang p on p.noCabang = a.parent where a.stsCabang = '2' order by p.namaCabang, a.namaCabang");
            }


            if (mySet.Tables[0].Rows.Count > 0)
            {
                GridViewBypass.DataSource = mySet;

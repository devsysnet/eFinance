...
{
    public partial class MstApprovalNilaiView : System.Web.UI.Page
    {
        ...
        private void SetInitialRow(string noApprove = "")
        {
            try
            {
                ...
                DataSet mySet = ObjDb.GetRows("SELECT * from MstApproveNilai where noApprove='" + noApprove + "'  ");
                foreach (DataRow myRow in mySet.Tables[0].Rows)
                {
                    dr = dt.NewRow();
                    dr["Jabatan"] = myRow["noAkses"].ToString();
        ...
        private void LoadData()
        {
            grdApprovalScheme.DataSource = ObjDb.GetRows("select * from MstApproveNilai a inner join " +
                ...
                "or b.namaParameterApprove LIKE '%" + txtSearch.Text + "%') and a.noCabang = '" + ObjSys.GetCabangId + "' ");

...
namespace eFinance.Pages.Master.View
{
    public partial class MstAgama : System.Web.UI.Page
    {
        ...
        private void SetPreviousData()
        {
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        TextBox txtAgama = (TextBox)grdAgama.Rows[i].FindControl("txtAgama");
        ...
        private void AddNewRow()
        {
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dtCurrentTable.Rows.Count > 0)
                {
                    ...
                    grdAgama.DataSource = dtCurrentTable;
                    grdAgama.DataBind();
                }
            }
            SetPreviousData();
        ...
        {
            grdGudang.DataSource = ObjDb.GetRows("select * from mAgama WHERE Agama LIKE '%" + txtSearch.Text + "%'");
            grdGudang.DataBind();
        }
        ...
        protected void grdGudang_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                grdAgama.DataSource = ObjDb.GetRows("select * from mAgama WHERE Agama ='" + hdnId.Value + "'");
                grdAgama.DataBind();
        ...
        {
            for (int i = 0; i < grdAgama.Rows.Count; i++)
            {
                TextBox txtAgama = (TextBox)grdAgama.Rows[i].FindControl("txtAgama");
                HiddenField hdnAgama = (HiddenField)grdAgama.Rows[i].FindControl("hdnAgama");

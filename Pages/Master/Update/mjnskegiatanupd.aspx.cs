...
{
    public partial class mjnskegiatanupd : System.Web.UI.Page
    {
        ...
        private void SetPreviousData()
        {
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        TextBox txtnamakegiatan = (TextBox)grdInstansi.Rows[i].FindControl("txtnamakegiatan");

                        txtnamakegiatan.Text = dt.Rows[i]["Column1"].ToString();
                    }
                }
            }
        }
            ...
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dtCurrentTable.Rows.Count > 0)
                {
                    for (int i = 0; i < dtCurrentTable.Rows.Count; i++)
                    {
                        ...
                        dtCurrentTable.Rows[i]["Column1"] = txtnamakegiatan.Text;
                    }
        ...
        {
            grdGudang.DataSource = ObjDb.GetRows("select * from mJenisKegiatan WHERE namakegiatan LIKE '%" + txtSearch.Text + "%'");
            grdGudang.DataBind();
        }
        ...
        protected void grdGudang_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                grdInstansi.DataSource = ObjDb.GetRows("select * from mJenisKegiatan WHERE noMkegiatan ='" + hdnId.Value + "'");
                grdInstansi.DataBind();
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (valid == true)
            {
                try
                {
                    ...
                    {
                        TextBox txtnamakegiatan = (TextBox)grdInstansi.Rows[i].FindControl("txtnamakegiatan");
                        HiddenField hdnnamakegiatan = (HiddenField)grdInstansi.Rows[i].FindControl("hdnnamakegiatan");

                        if (txtnamakegiatan.Text != "")
                        {
                            ObjDb.Data.Clear();
                            ObjDb.Where.Clear();
                            ObjDb.Where.Add("noMkegiatan", hdnnamakegiatan.Value);
                            ObjDb.Data.Add("namaKegiatan", txtnamakegiatan.Text);

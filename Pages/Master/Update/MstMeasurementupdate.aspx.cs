...
{
    public partial class MstMeasurementupdate : System.Web.UI.Page
    {
        ...
        protected void grdGudang_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                grdInstansi.DataSource = ObjDb.GetRows("select * from mMesurement WHERE mesurement ='" + hdnId.Value + "'");
                grdInstansi.DataBind();
            ...
            try
            {
                if (hdnMode.Value.ToLower() == "deleteall")
                {
                    ...
                    if (arrayItem != null)
                    {
                        ...
                        {
                            ObjDb.Where.Clear();
                            ObjDb.Where.Add("mesurement", itemRow);
                            ObjDb.Delete("mMesurement", ObjDb.Where);
                        }
                    }
                    foreach (GridViewRow gvrow in grdGudang.Rows)
                    {
                        ...
                        {
                            ObjDb.Where.Clear();
                            ObjDb.Where.Add("mesurement", itemId);
                            ObjDb.Delete("mMesurement", ObjDb.Where);
                        }
                    }
                }
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (valid == true)
            {
                try
                {
                    ...
                    {
                        TextBox txtmeasurement = (TextBox)grdInstansi.Rows[i].FindControl("txtmeasurement");
                        HiddenField hdnmeasurement = (HiddenField)grdInstansi.Rows[i].FindControl("hdnmeasurement");

                        if (txtmeasurement.Text != "")
                        {
                            ObjDb.Data.Clear();
                            ObjDb.Where.Clear();
                            ObjDb.Where.Add("mesurement", hdnmeasurement.Value);
                            ObjDb.Data.Add("mesurement", txtmeasurement.Text);
                            ObjDb.Data.Add("modiBy", ObjSys.GetUserId);
                            ObjDb.Data.Add("modiDate", ObjSys.GetNow);
                            ObjDb.Update("mMesurement", ObjDb.Data, ObjDb.Where);

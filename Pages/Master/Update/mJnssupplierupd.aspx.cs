...
{
    public partial class mJnssupplierupd : System.Web.UI.Page
    {
        ...
        private void SetPreviousData()
        {
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        TextBox txtjnssupplier = (TextBox)grdInstansi.Rows[i].FindControl("txtjnssupplier");

                        txtjnssupplier.Text = dt.Rows[i]["Column1"].ToString();
                    }
                }
            }
        }
            ...
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dtCurrentTable.Rows.Count > 0)
                {
                    for (int i = 0; i < dtCurrentTable.Rows.Count; i++)
                    {
                        ...
                        dtCurrentTable.Rows[i]["Column1"] = txtjnssupplier.Text;
                    }
        ...
        protected void grdGudang_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string nojnssup = grdGudang.DataKeys[rowIndex].Values[0].ToString();
                hdnId.Value = nojnssup;
                CloseMessage();


                grdInstansi.DataSource = ObjDb.GetRows("select * from mJenissupplier WHERE Jenissup ='" + hdnId.Value + "'");
                grdInstansi.DataBind();
            ...
            try
            {
                if (hdnMode.Value.ToLower() == "deleteall")
                {
                    ...
                    if (arrayItem != null)
                    {
                        ...
                        {
                            ObjDb.Where.Clear();
                            ObjDb.Where.Add("Jenissup", itemRow);
                            ObjDb.Delete("mJenissupplier", ObjDb.Where);
                        }
                    }
                    foreach (GridViewRow gvrow in grdGudang.Rows)
                    {
                        ...
                        {
                            ObjDb.Where.Clear();
                            ObjDb.Where.Add("Jenissup", itemId);
                            ObjDb.Delete("mJenissupplier", ObjDb.Where);
                        }
                    }
                }
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (valid == true)
            {
                try
                {
                    ...
                    {
                        TextBox txtjnssupplier = (TextBox)grdInstansi.Rows[i].FindControl("txtjnssupplier");
                        HiddenField hdnjnssupplier = (HiddenField)grdInstansi.Rows[i].FindControl("hdnjnssupplier");

                        if (txtjnssupplier.Text != "")
                        {
                            ObjDb.Data.Clear();
                            ObjDb.Where.Clear();
                            ObjDb.Where.Add("Jenissup", hdnjnssupplier.Value);
                            ObjDb.Data.Add("Jenissup", txtjnssupplier.Text);
                            ObjDb.Data.Add("modiBy", ObjSys.GetUserId);
                            ObjDb.Data.Add("modiDate", ObjSys.GetNow);
                            ObjDb.Update("mJenissupplier", ObjDb.Data, ObjDb.Where);

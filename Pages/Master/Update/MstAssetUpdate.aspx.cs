...
{
    public partial class MstAssetUpdate : System.Web.UI.Page
    {
        ...
        {
            grdAsset.DataSource = ObjDb.GetRows("select nomAsset,CASE kategori WHEN 1 THEN 'Bangunan' WHEN 2 THEN 'Non Bangunan' END kategori,kelompok,tahun,CASE jenis WHEN 1 THEN 'Garis Lurus' WHEN 2 THEN 'Saldo Menurun' END jenis,tarif from mAsset where kelompok LIKE '%" + txtSearch.Text + "%' and nomAsset not in(select kelompokAsset from mBarang where jenisBrg=1)");
            grdAsset.DataBind();
        }
        ...
        protected void btnSimpan_Click(object sender, EventArgs e)
        {
            ...
            if (valid == true)
            {
                try
                {
                    ObjDb.Data.Clear();
                    ObjDb.Where.Clear();
                    ObjDb.Where.Add("nomAsset", hdnId.Value);
                    ObjDb.Data.Add("kategori", cboBangunan.Text);
                    ObjDb.Data.Add("kelompok", txtKelompok.Text);
                    ObjDb.Data.Add("modiBy", ObjSys.GetUserId);
                    ObjDb.Data.Add("modidate", ObjSys.GetNow);
                    ObjDb.Data.Add("tahun", txtMasa.Text);
                    ObjDb.Data.Add("jenis", cboJenis.Text);
                    ObjDb.Data.Add("tarif", txtTarif.Text);
                    ObjDb.Update("mAsset", ObjDb.Data, ObjDb.Where);

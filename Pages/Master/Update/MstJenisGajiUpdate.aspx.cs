...
{
    public partial class MstJenisGajiUpdate : System.Web.UI.Page
    {
        ...
        private void SetPreviousData()
        {
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        TextBox txtKota = (TextBox)grdInstansi.Rows[i].FindControl("txtKota");
        ...
        {
            grdGudang.DataSource = ObjDb.GetRows("select * from mstjenisGaji WHERE nama LIKE '%" + txtSearch.Text + "%'");
            grdGudang.DataBind();
        }
        ...
        protected void grdGudang_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                grdInstansi.DataSource = ObjDb.GetRows("select * from mstjenisGaji WHERE nama ='" + hdnId.Value + "'");
                grdInstansi.DataBind();
            ...
            try
            {
                if (hdnMode.Value.ToLower() == "deleteall")
                {
                    ...
                    if (arrayItem != null)
                    {
                        ...
                        {
                            ObjDb.Where.Clear();
                            ObjDb.Where.Add("nama", itemRow);
                            ObjDb.Delete("mstjenisGaji", ObjDb.Where);
                        }
                    }
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (valid == true)
            {
                try
                {
                    ...
                    {
                        TextBox txtKota = (TextBox)grdInstansi.Rows[i].FindControl("txtKota");
                        HiddenField hdnKota = (HiddenField)grdInstansi.Rows[i].FindControl("hdnKota");

                        if (txtKota.Text != "")
                        {
                            ObjDb.Data.Clear();
                            ObjDb.Where.Clear();
                            ObjDb.Where.Add("nama", hdnKota.Value);
                            ObjDb.Data.Add("nama", txtKota.Text);
                            ObjDb.Data.Add("modiBy", ObjSys.GetUserId);
                            ObjDb.Data.Add("modiDate", ObjSys.GetNow);
                            ObjDb.Update("mstjenisGaji", ObjDb.Data, ObjDb.Where);

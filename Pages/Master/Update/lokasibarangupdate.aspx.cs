...
{
    public partial class lokasibarangupdate : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                //ObjSys.SessionCheck("MstProductLocationUpdate.aspx");
                loadData();
            }
        }
        ...
        {
            grdVoucher.DataSource = ObjDb.GetRows("select distinct *  from mBarang WHERE sts='1' and jenisbrg='5' and (kodebarang LIKE '%" + txtSearch.Text + "%' or namabarang LIKE '%" + txtSearch.Text + "%') and noBarang in (select noBarang mLokasiBarang)");
            grdVoucher.DataBind();
        }
            ...
            try
            {
                if (hdnMode.Value.ToLower() == "deleteall")
                {
                    ...
                    if (arrayItem != null)
                    {
                        ...
                        {
                            ObjDb.Where.Clear();
                            ObjDb.Where.Add("noBarang", itemRow);
                    ...
                    foreach (GridViewRow gvrow in grdVoucher.Rows)
                    {
                        ...
                        {
                            ObjDb.Where.Clear();
                            ObjDb.Where.Add("noBarang", itemId);
                            ObjDb.Delete("mLokasiBarang", ObjDb.Where);
                        }
                    }
                }
            ...
            if (valid == true)
            {
                try
                {
                    for (int i = 0; i < grdProdLoc.Rows.Count; i++)
                    {
                        ...
                        HiddenField hdnnoBarang = (HiddenField)grdProdLoc.Rows[i].FindControl("hdnnoBarang");

                        ObjDb.Data.Clear();
                        ObjDb.Where.Clear();
                        ObjDb.Where.Add("noLokasiBarang", hdnnoBarang.Value);
        ...
        protected void grdVoucher_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                CloseMessage();
                DataSet MySet = ObjDb.GetRows("select * from mLokasiBarang a inner join Gudang b on a.noGudang = b.noGudang inner join mbarang d on a.noBarang = d.noBarang WHERE a.noBarang ='" + noLokasiBarang + "'");
                if (MySet.Tables[0].Rows.Count > 0)
                {
                    ...
                    grdProdLoc.DataSource = ObjDb.GetRows("select * from mLokasiBarang a inner join Gudang b on a.noGudang = b.noGudang inner join mbarang d on a.noBarang = d.noBarang WHERE a.noBarang ='" + hdnId.Value + "'");
                    grdProdLoc.DataBind();

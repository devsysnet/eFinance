...
{
    public partial class MstPertanyaan : System.Web.UI.Page
    {
        ...
        protected void LoadDataParent()
        {
            ...
            grdRekening.DataSource = ObjGlobal.GetDataProcedure("SPLoadDataSoalKPI", ObjGlobal.Param);
        ...
        protected void loadDataCombo()
        {
            ...
            cboCabang.DataSource = ObjDb.GetRows("select a.* from (select 0 as id,'---Pilih Cabang---' as name ,0 as urutan union SELECT distinct nocabang id, namaCabang name, noUrut as urutan FROM vCabang) a order by urutan");
        ...
        protected void btnSimpan_Click(object sender, EventArgs e)
        {
            ...
            {
                message += ObjSys.CreateMessage("Kode Soal harus diisi");
                valid = false;
            }
            if (txtAccountCode.Text != "")
            {
                ObjGlobal.Param.Clear();
                ObjGlobal.Param.Add("kodesoal", txtAccountCode.Text);
                if (ObjGlobal.GetDataProcedure("SPLoadDataSoalKPID", ObjGlobal.Param).Tables[0].Rows.Count > 1)
                {
                    message += ObjSys.CreateMessage("Soal sudah ada.");
                    valid = false;
                }
            }
            ...
            if (txtRemark.Text == "")
            {
                message += ObjSys.CreateMessage("Keterangan harus diisi.");
                valid = false;
            }
            ...
            try
            {
                if (valid == true)
                {
                    ...
                    ObjGlobal.Param.Clear();
                    ObjGlobal.Param.Add("kodesoal", txtAccountCode.Text);
                    ObjGlobal.Param.Add("Ketsoal", txtRemark.Text);
                    if (txtParent.Text != "")
                    {
                    ...
                    if (hdnID.Value != "")
                    {
                        DataSet mySet_CekTingkat = ObjDb.GetRows("select * from mSoalKPI where nosoal = '" + hdnID.Value + "'");
                    ...
                    ObjGlobal.Param.Add("jenis", cboType.Text);
                    ObjGlobal.Param.Add("createdBy", ObjSys.GetUserId);

                    ObjGlobal.ExecuteProcedure("SPInsertSoalKPI", ObjGlobal.Param);

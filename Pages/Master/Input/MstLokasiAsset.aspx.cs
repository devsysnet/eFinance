...
{
    public partial class MstLokasiAsset : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ObjSys.SessionCheck("MstLokasiAsset.aspx");
                LoadDataSearch();
                SetInitialRow();
                for (int i = 1; i < 5; i++)
                {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (valid == true)
            {
                try
                {
                    if (ObjDb.GetRows("select * from mLokasi where Lokasi = '" + txtBrand.Text + "' and nocabang='" + ObjSys.GetCabangId + "'").Tables[0].Rows.Count > 0)
                    {
                        ScriptManager.RegisterClientScriptBlock(this, Page.GetType(), "ToTheTop", "ToTopOfPage();", true);

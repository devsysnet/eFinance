...
{
    public partial class MstKursPajak : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ObjSys.SessionCheck("MstKursPajak.aspx");
                SetInitialRow();
            }
        }
            ...
            if (valid == true)
            {
                try
                {
                    for (int i = 0; i < grdInstansi.Rows.Count; i++)
                    {
                        TextBox txtInstansi = (TextBox)grdInstansi.Rows[i].FindControl("txtInstansi");
                        HiddenField hdnCurrency = (HiddenField)grdInstansi.Rows[i].FindControl("hdnCurrency");
                        DataSet mySet = ObjDb.GetRows("Select * from tKursPajak where tglKursPajak = '" + dtTgl1.Text + "' and tgl = '" + dtTgl2.Text + "' and noMataUang='" + hdnCurrency.Value + "'");
                        if (mySet.Tables[0].Rows.Count > 0)
                        {
                        ...
                        else
                        {
                            if (txtInstansi.Text != "")
                            {
                                ...
                                ObjDb.Data.Add("tglKursPajak", dtTgl1.Text);
                                ObjDb.Data.Add("nilaiKursPajak", Convert.ToDecimal(txtInstansi.Text).ToString());
                                ObjDb.Data.Add("createdBy", ObjSys.GetUserId);
                                ObjDb.Data.Add("createdDate", ObjSys.GetNow);
                                ObjDb.Data.Add("tgl", dtTgl2.Text);
                                ObjDb.Data.Add("Catatan", txtNoKeputusan.Text);
                                ObjDb.Insert("tKursPajak", ObjDb.Data);

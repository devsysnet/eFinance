...
{
    public partial class mstStatusjenisPeg : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ObjSys.SessionCheck("mstStatusjenisPeg.aspx");
                SetInitialRow();
                for (int i = 1; i < 5; i++)
                {
                    AddNewRow();
            ...
            if (valid == true)
            {
                try
                {
                    for (int i = 0; i < grdInstansi.Rows.Count; i++)
                    {
                        ...
                        if (ObjDb.GetRows("SELECT * FROM Mstjenispegawai WHERE statuspegawai = '" + txtAgama.Text + "'").Tables[0].Rows.Count > 0)
                        {
                        ...
                        else
                        {
...

                            {
                                ...
                                ObjDb.Insert("Mstjenispegawai", ObjDb.Data);

...
{
    public partial class lokasibarang : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ObjSys.SessionCheck("lokasibarang.aspx");
            }
        }
            ...
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        TextBox txtMinStok = (TextBox)grdProdLoc.Rows[i].FindControl("txtMinStok");
                        ...
                        txtMinStok.Text = dt.Rows[i]["Column3"].ToString();
                    }
                }
            }
        }
            ...
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dtCurrentTable.Rows.Count > 0)
                {
                    for (int i = 0; i < dtCurrentTable.Rows.Count; i++)
                    {
                        ...
                        dtCurrentTable.Rows[i]["Column1"] = cboGudang.Text;
                        dtCurrentTable.Rows[i]["Column2"] = box4.Text;
                        dtCurrentTable.Rows[i]["Column3"] = txtMinStok.Text;
        ...
        {
            grdVoucher.DataSource = ObjDb.GetRows("SELECT * from mbarang where sts = '1' and jenisbrg='5' and nobarang not in(select nobarang from mlokasibarang) AND (kodebarang LIKE '%" + txtSearch.Text + "%' or namabarang LIKE '%" + txtSearch.Text + "%') ");
            grdVoucher.DataBind();
        }
                ...
                try
                {
                    ...
                    //{
                        for (int i = 0; i < grdProdLoc.Rows.Count; i++)
                        {
                            ...
                            {
                                ObjDb.Data.Clear();
                                ObjDb.Data.Add("nobarang", hdnnobarang.Value);
        ...
        protected void cboLokasiGudang_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < grdProdLoc.Rows.Count; i++)
                {
                    ...
                    //DataSet MySet = ObjDb.GetRows("select * from mLokasiBarang where nobarang = " + hdnnobarang.Value + " and noLokasiGudang ='" + LokasiGudangGrid.SelectedValue + "'");

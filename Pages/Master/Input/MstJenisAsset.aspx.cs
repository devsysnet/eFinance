...
{
    public partial class MstJenisAsset : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ObjSys.SessionCheck("MstJenisAsset.aspx");
                SetInitialRow();
                for (int i = 1; i < 5; i++)
                {
                    AddNewRow();
            ...
            if (valid == true)
            {
                try
                {
                    for (int i = 0; i < grdInstansi.Rows.Count; i++)
                    {
                        ...
                        if (ObjDb.GetRows("SELECT * FROM MstJenisasset WHERE Jenis = '" + txtjnsAsset.Text + "'").Tables[0].Rows.Count > 0)
                        {
                            message += ObjSys.CreateMessage("Baris " + (i + 1) + " <b>" + txtjnsAsset.Text + "</b> tidak tersimpan karena nama Jenis Asset sudah terdaftar sebelumnya.");
                            count2 += 1;
                        }
                        else
                        {
...

                            {
                                ...
                                ObjDb.Insert("MstJenisasset", ObjDb.Data);
                                message += ObjSys.CreateMessage("Baris " + (i + 1) + " <b>" + txtjnsAsset.Text + "</b> data tersimpan.");
                                count += 1;
                            }
                        }

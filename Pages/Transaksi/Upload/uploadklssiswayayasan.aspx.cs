...
{
    public partial class uploadklssiswayayasan : System.Web.UI.Page
    {
            ...
            try
            {
                if (flUpload.HasFile == true)
                {
                    ...
                    foreach (DataRow row2 in dt.Rows)
                    {
                        ...
                        string nocabangx = Convert.ToString(row2["nocabang"]);
                        ...
                        using (SqlConnection con = new SqlConnection(ObjDb.ConDb()))
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand("SPProcessImportkelas"))
                            {
                                ...
                                cmd.Parameters.AddWithValue("@noCabang", nocabangx);
            ...
            try
            {
                if (valid == true)
                {
                    using (SqlConnection con = new SqlConnection(ObjDb.ConDb()))
                    {
                        ...
                        try
                        {
                            cmd = new SqlCommand("SPProcessExportTranskelasYayasan", con);
                            cmd.Parameters.Add(new SqlParameter("@cabang", cboUnit.Text));
                            cmd.Parameters.Add(new SqlParameter("@perwakilan", ObjSys.GetCabangId));
                            cmd.CommandType = CommandType.StoredProcedure;

...
{
    public partial class ExportImportTableTransaksi : System.Web.UI.Page
    {
        ...
        protected void loadData()
        {
            ...
            cboMonth.DataSource = ObjGlobal.GetDataProcedure("SPLoadBulanSaldoBln1", ObjGlobal.Param);
            cboMonth.DataValueField = "bln";
            cboMonth.DataTextField = "bulan";
            cboMonth.DataBind();

            cboYear.DataSource = ObjGlobal.GetDataProcedure("SPLoadTahunSaldoBln1", ObjGlobal.Param);
            cboYear.DataValueField = "thn";
            cboYear.DataTextField = "thn";
            cboYear.DataBind();

            cboTabel.DataSource = ObjGlobal.GetDataProcedure("SPLoadTabelExportTransaksi");
            ...
            try
            {
                if (valid == true)
                {
                    using (SqlConnection con = new SqlConnection(ObjDb.ConDb()))
                    {
                        ...
                        try
                        {
                            cmd = new SqlCommand("SPProcessExport", con);

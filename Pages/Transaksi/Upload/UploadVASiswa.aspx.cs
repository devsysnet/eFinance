...
{
    public partial class UploadVASiswa : System.Web.UI.Page
    {
        ...
        protected void btnImport_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (flUpload.HasFile == true)
                {
                    ...
                    string excelPath = Server.MapPath("~/Assets/UploadVA/") + Path.GetFileName(flUpload.PostedFile.FileName);
                    ...
                    foreach (DataRow row2 in dt.Rows)
                    {
                        ...
                        string NoVA = Convert.ToString(row2["NoVA"]);
                        string kategori = Convert.ToString(row2["kategori"]);                     
                        string kdCabang = Convert.ToString(row2["kdCabang"]);
                        ...
                        using (SqlConnection con = new SqlConnection(ObjDb.ConDb()))
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand("SPProcessImportVA"))
                            {
                                ...
                                cmd.Parameters.AddWithValue("@kdCabang", kdCabang);
                ...
                if (valid == true)
                {
                    using (SqlConnection con = new SqlConnection(ObjDb.ConDb()))
                    {
                        ...
                        try
                        {
                            ...
                            cmd = new SqlCommand("SPProcessExportDataVA", con);
                            cmd.Parameters.Add(new SqlParameter("@cabang", ObjSys.GetCabangId));
                            cmd.Parameters.AddWithValue("@kategori", cboStatus.Text);
                            cmd.CommandType = CommandType.StoredProcedure;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=dataVA " + ObjSys.GetNow + ".csv");
                            Response.Charset = "";

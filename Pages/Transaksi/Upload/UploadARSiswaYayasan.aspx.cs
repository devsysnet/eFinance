...
{
    public partial class UploadARSiswaYayasan : System.Web.UI.Page
    {
        ...
        private void loadDataCombo()
        {
            ...
            loadDataCombocbo(cboUnit.Text);
        }
        private void loadDataCombocbo(string unit = "0")
        {
            if (unit == "0")
            {
            ...
            {
                cboKategori.DataSource = ObjDb.GetRows("select * from (select '0' as id, '---Pilih---' name union select noTransaksi as id, jenisTransaksi as name from mJenisTransaksi where nocabang='" + unit + "' and posbln<>'1')x");
                cboKategori.DataValueField = "id";
                cboKategori.DataTextField = "name";
                cboKategori.DataBind();

                cboTA.DataSource = ObjDb.GetRows("select * from (select '' as id, '---Semua---' name union select distinct tahunAjaran as id, tahunAjaran as name from TransKelas where nocabang='" + unit + "')x");
                cboTA.DataValueField = "id";
                cboTA.DataTextField = "name";
                cboTA.DataBind();
            }
                ...
                try
                {
                    ...
                    if (flUpload.HasFile == true)
                    {
                        ...
                        if (cboflUpload.Text == "csv/txt")
                        {
                            ...
                            ObjGlobal.GetDataProcedure("UploadDataARSiswa_tKasDetilyayasan", ObjGlobal.Param);
        ...
        public void MyCSV()
        {
            ...
            DataSet mySet = ObjGlobal.GetDataProcedure("spdownloaddtsiswayayasan", ObjGlobal.Param);

...
{
    public partial class uploadPendapatanbln : System.Web.UI.Page
    {
        ...
        {
            cboYear.DataSource = ObjDb.GetRows("select a.* from (select '0' id,'-' name union all SELECT distinct nokomponengaji id, komponengaji name FROM mstkomponenGaji where jenis = '0' and kategori='1') a");
                ...
                if (flUpload.HasFile == true)
                {
                    ...
                    foreach (DataRow row2 in dt.Rows)
                    {
                        ...
                        using (SqlConnection con = new SqlConnection(ObjDb.ConDb()))
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand("SPProcessImportPendapatan"))
                            {

                                cmd.CommandType = CommandType.StoredProcedure;
                                cmd.Parameters.AddWithValue("@noCabang", nocabang);
                                cmd.Parameters.AddWithValue("@createdBy", ObjSys.GetUserId);
                                cmd.Parameters.AddWithValue("@tgl", dtpendapatan.Text);
                                cmd.Parameters.AddWithValue("@jenis", cboYear.Text);

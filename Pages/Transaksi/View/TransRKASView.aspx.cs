...
{
    public partial class TransRKASView : System.Web.UI.Page
    {
        ...
        protected void LoadDataCabang()
        {
            ...
            {
                cboCabang.DataSource = ObjDb.GetRows("select a.* from (select 0 as id,'---Pilih Cabang---' as name Union SELECT distinct nocabang id,namaCabang name FROM mcabang where stsCabang in (2,3,4) and parent = " + ObjSys.GetCabangId + " and noCabang in (select distinct noCabang from TransRKAS_H)) a");
            ...
            {
                cboCabang.DataSource = ObjDb.GetRows("select a.* from (SELECT distinct nocabang id,namaCabang name FROM mcabang where nocabang='" + ObjSys.GetCabangId + "' and noCabang in (select distinct noCabang from TransRKAS_H)) a");
                cboCabang.DataValueField = "id";
                cboCabang.DataTextField = "name";
                cboCabang.DataBind();
            }
            ...
            {
                cboCabang.DataSource = ObjDb.GetRows("select a.* from (select 0 as id,'---Pilih Cabang---' as name Union SELECT distinct nocabang id,namaCabang name FROM mcabang where stsCabang in (2,3,4) and parent = " + ObjSys.GetParentCabang + " and noCabang in (select distinct noCabang from TransRKAS_H) ) a");
                cboCabang.DataValueField = "id";
                cboCabang.DataTextField = "name";
                cboCabang.DataBind();
            }
            ...
            {
                cboCabang.DataSource = ObjDb.GetRows("select a.* from (select 0 as id,'---Pilih Cabang---' as name Union SELECT distinct nocabang id,namaCabang name FROM mcabang where stsCabang in (2,3,4) and noCabang in (select distinct noCabang from TransRKAS_H) ) a");
                cboCabang.DataValueField = "id";
                cboCabang.DataTextField = "name";
                cboCabang.DataBind();
            }

            LoadDataTahun(cboCabang.Text);
        }
        protected void LoadDataTahun(string cabang = "")
        {
            cboThn.DataSource = ObjDb.GetRows("SELECT DISTINCT id, name FROM ( "+
                "SELECT '0' id, ' ---Pilih Tahun--- ' name "+
                "union all "+
                "SELECT CONVERT(varchar,tahun) id, CONVERT(varchar,tahun) name FROM TransRKAS_H WHERE noCabang = " + cabang + " "+
                ") x");
        ...
        {
            btnProses.Visible = Proses;
            btnReset.Visible = Reset;
        ...
        {
            loadDataFirst(cboThn.Text, cboCabang.Text);
        }
        ...
        {
            CloseMessage();
            DataSet mySet = ObjDb.GetRows("SELECT noTRKAS FROM TransRKAS_H where tahun = " + cboThn.Text + " and noCabang = " + cboCabang.Text + "");
            DataRow myRow = mySet.Tables[0].Rows[0];
            string noTRKAS = myRow["noTRKAS"].ToString();
            ...
            DataSet mySetH = ObjDb.GetRows("Select a.noTRKAS, b.noRek from TransRKAS_H a inner join TransRKAS_AD b on a.noTRKAS = b.noTRKAS "+
                "Where a.tahun = " + cboThn.Text + " and a.noCabang = " + cboCabang.Text + " and b.noRek = " + cboAkun.Text + "");
            if (mySetH.Tables[0].Rows.Count > 0)
            {

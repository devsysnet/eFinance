...
{
    public partial class Rdetailtagihanpercabangpercoa : System.Web.UI.Page
    {
            ...
            try
            {
                if (valid == true)
                {
                    using (SqlConnection con = new SqlConnection(ObjDb.ConDb()))
                    {
                        ...
                        try
                        {
                            cmd = new SqlCommand("SPRReportdetailbayarpercoacabang", con);
                            ...
                            cmd.Parameters.Add(new SqlParameter("@jnstrs", cbojnstrs.Text));
                            cmd.CommandType = CommandType.StoredProcedure;
                            da.SelectCommand = cmd;
                            da.Fill(dt);

                            String fileName = "Detail_Pembayaran_Cabang" + ObjSys.GetNow + ".xls";
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            ObjGlobal.Param.Add("jnstrs", cbojnstrs.Text);
            tabelDinamis(ObjGlobal.GetDataProcedure("SPRReportdetailbayarpercoacabang", ObjGlobal.Param));
        }
        ...
        protected void loadDataCombo()
        {
            ...
            else if (ObjSys.GetstsCabang == "0" && ObjSys.GetstsPusat == "3")
            {
                ...
                cboCabang.DataSource = ObjDb.GetRows("select a.* from (SELECT distinct b.nocabang id, namaCabang name FROM vCabang b inner join (select parent from mcabang where nocabang = '" + ObjSys.GetCabangId + "') c on b.parent=c.parent and stscabang=3) a ");
                cboCabang.DataValueField = "id";
                cboCabang.DataTextField = "name";
                cboCabang.DataBind();
            }
            ...
            {
                cboCabang.DataSource = ObjDb.GetRows("select a.* from (SELECT distinct b.nocabang id, namaCabang name FROM vCabang b inner join (select nocabang from mcabang where nocabang = '" + ObjSys.GetCabangId + "') c on b.parent=c.nocabang and stscabang=3) a ");
                cboCabang.DataValueField = "id";
                cboCabang.DataTextField = "name";
                cboCabang.DataBind();
            }

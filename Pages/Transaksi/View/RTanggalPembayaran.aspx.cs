...
{
    public partial class RTanggalPembayaran : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                loadDatathnajaran();
                LoadIndexYayasan();
                cbothnajaran.DataSource = ObjDb.GetRows("select a.* from (select '0' id,'--Pilih Tahun Ajaran--' name union  SELECT distinct tahunajaran id, tahunajaran name from TransPiutang where tahunajaran != '') a");
        ...
        protected void btnPosting_Click(object sender, EventArgs e)
        {
            ...
            grdAccount.DataSource = ObjGlobal.GetDataProcedure("SPViewtanggalpembayaran", ObjGlobal.Param);
            grdAccount.DataBind();
            ...
            try
            {
                if (valid == true)
                {
                    using (SqlConnection con = new SqlConnection(ObjDb.ConDb()))
                    {
                        ...
                        try
                        {
                            cmd = new SqlCommand("SPViewtanggalpembayaran", con);

...
{
    public partial class RrekappostingAR : System.Web.UI.Page
    {
            ...
            try
            {
                if (valid == true)
                {
                    using (SqlConnection con = new SqlConnection(ObjDb.ConDb()))
                    {
                        ...
                        try
                        {
                            cmd = new SqlCommand("SPLoadTrackingposting", con);
                            cmd.Parameters.Add(new SqlParameter("@nocabang", cboCabang.Text));
                            cmd.CommandType = CommandType.StoredProcedure;
                            da.SelectCommand = cmd;
                            da.Fill(dt);

                            String fileName = "Rekap_posting_AR" + ObjSys.GetNow + ".xls";
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            tabelDinamis(ObjGlobal.GetDataProcedure("SPLoadTrackingposting", ObjGlobal.Param));
        }
        ...
        protected void loadDataCombo()
        {
            ...
            {
                cboCabang.DataSource = ObjDb.GetRows("select a.* from (SELECT distinct nocabang id, namaCabang name FROM vCabang b inner join (select parent from mcabang where nocabang = '" + ObjSys.GetCabangId + "') c on b.parent=c.parent and stscabang=1) a ");
                cboCabang.DataValueField = "id";
                cboCabang.DataTextField = "name";
                cboCabang.DataBind();
            }

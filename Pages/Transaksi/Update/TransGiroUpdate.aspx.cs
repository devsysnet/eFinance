...
{
    public partial class TransGiroUpdate : System.Web.UI.Page
    {
        ...
        {
            cborekeningbank.DataSource = ObjDb.GetRows("select a.* from (select 0 as id,'-- Pilih Rekening --' as name union all SELECT distinct norek id, ket name FROM mrekening where jenis=2 and stsaktif=1 and nocabang='" + ObjSys.GetCabangId + "' ) a");
        ...
        protected void LoadData()
        {
            ...
            grdAssetUpdate.DataSource = ObjDb.GetRows(" SELECT a.nodeposito,a.kdTransaksi,a.nomor,a.NamaBank,a.tglDeposito,a.tglJatuhTempo,b.Jenis, CASE WHEN a.Jenis = 1 THEN 'Saldo Awal' WHEN a.jenis = 2 THEN 'Pengeluaran Bank' ELSE 'Lain-Lain' END AS jenisTransaksi FROM Tdeposito a inner join MstJenisasset b on a.jenis = b.nojnsasset where a. nocabang='" + ObjSys.GetCabangId + "'and ( kdTransaksi Like '%" + txtSearch.Text + "%' OR nomor Like '%" + txtSearch.Text + "%' )");
            grdAssetUpdate.DataBind();
        }
        ...
        protected void btnSimpan_Click(object sender, EventArgs e)
        {
            ...
            if (valid == true)
            {
                try
                {
                    ...
                    ObjDb.Data.Add("jenistransaksi", cbojenistransaksi.Text);
                    ObjDb.Data.Add("norekbank", cborekeningbank.Text);
                    ObjDb.Data.Add("Jenis", cbojenissrt.Text);
                    ObjDb.Data.Add("nomor", nomorGiro.Text);
                    ObjDb.Data.Add("Namabank", namabank.Text);
                    ObjDb.Data.Add("tglDeposito", tglGiro.Text);
                    ObjDb.Data.Add("tglJatuhTempo", tglJatuhTempo.Text);
                    ObjDb.Data.Add("nominal", Convert.ToDecimal(nominal.Text).ToString());
                    ObjDb.Data.Add("jnsbunga", cbojnsbunga.Text);
                    ObjDb.Data.Add("bunga", Convert.ToDecimal(bunga.Text).ToString());
                    ObjDb.Data.Add("norek", cboRek.Text);
                    ObjDb.Data.Add("deskripsi", deskripsi.Text);
                    ObjDb.Data.Add("modiBy", ObjSys.GetUserId);
                    ObjDb.Data.Add("modidate", ObjSys.GetDate);
                    ObjDb.Update("Tdeposito", ObjDb.Data, ObjDb.Where);
        ...
        protected void grdAssetUpdate_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                if (e.CommandName == "SelectEdit")
                {
                    ...
                    DataSet mySet = ObjDb.GetRows("select nodeposito,kdTransaksi,jenistransaksi,norekbank,Jenis,nomor,tglDeposito,tglJatuhTempo,nominal,NamaBank,bunga,jnsbunga,isnull(norek,0) as norek,deskripsi " +
                    ...
                    cbojnsbunga.Text= myRow["jnsbunga"].ToString();
                    bunga.Text= ObjSys.IsFormatNumber(myRow["bunga"].ToString());

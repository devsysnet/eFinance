...
{
    public partial class updateJurnalMemoCabang : System.Web.UI.Page
    {
        ...
        private void SetInitialRow(string kd = "")
        {
            ...
            DataSet mySetHx = ObjDb.GetRows("select namacabang from mcabang where nocabang = '" + cboCabang.Text + "'");
            DataRow myRowHx = mySetHx.Tables[0].Rows[0];
            ...
            dr = dt.NewRow();
            DataSet mySet = ObjDb.GetRows("select * from tKasDetil a inner join mRekening b on a.noRek = b.noRek inner join mcabang c on a.nocabang = c.nocabang WHERE a.kdTran = '" + kd + "' ");
            foreach (DataRow myRow in mySet.Tables[0].Rows)
            {
                ...
                dr["Column4"] = ObjSys.IsFormatNumber(myRow["Debet"].ToString());
                dr["Column5"] = ObjSys.IsFormatNumber(myRow["Kredit"].ToString());
                dr["Column6"] = myRow["Uraian"].ToString();
                dr["Column7"] = myRow["noTkasDetil"].ToString();
                dr["Column8"] = myRow["nocabang"].ToString();
                dt.Rows.Add(dr);
            }
            ...
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        DropDownList cbocabangunit = (DropDownList)grdMemoJurnal.Rows[i].FindControl("cbocabangunit");

                        DataSet MySetX = ObjDb.GetRows("select * from mcabang a where nocabang = '" + cboCabang.Text + "'");

 
                            DataRow MyRowX = MySetX.Tables[0].Rows[0];

                            cbocabangunit.DataSource = ObjDb.GetRows("select nocabang id, namacabang name from mcabang where parent = '" + MyRowX["parent"].ToString() +"'");
                        cbocabangunit.DataValueField = "id";
                        cbocabangunit.DataTextField = "name";
                        cbocabangunit.DataBind();

                        cbocabangunit.Text = dt.Rows[i]["Column8"].ToString();
                        ...
                        txtRemark.Text = dt.Rows[i]["Column6"].ToString();
                        if (dt.Rows[i]["Column4"].ToString() == "")
                            txtDebit.Text = "0.00";
                        else
                            txtDebit.Text = dt.Rows[i]["Column4"].ToString();
                        if (dt.Rows[i]["Column5"].ToString() == "")
                            txtKredit.Text = "0.00";
                        else
                            txtKredit.Text = dt.Rows[i]["Column5"].ToString();
            ...
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dtCurrentTable.Rows.Count > 0)
                {
                    for (int i = 0; i < dtCurrentTable.Rows.Count; i++)
                    {
                        ...
                        dtCurrentTable.Rows[i]["Column4"] = txtDebit.Text;
                        dtCurrentTable.Rows[i]["Column5"] = txtKredit.Text;
                        dtCurrentTable.Rows[i]["Column6"] = txtRemark.Text;
        ...
        //                {
        //                    ObjDb.Where.Clear();
        //                    ObjDb.Where.Add("nomorKode", itemRow);
        //                    ObjDb.Delete("tKas", ObjDb.Where);

        //                    ObjDb.Where.Clear();
        //                    ObjDb.Where.Add("kdTran", itemRow);
        ...
        //                {
        //                    ObjDb.Where.Clear();
        //                    ObjDb.Where.Add("nomorKode", itemId);
        //                    ObjDb.Delete("tKas", ObjDb.Where);

        //                    ObjDb.Where.Clear();
        //                    ObjDb.Where.Add("kdTran", itemId);
        //                    ObjDb.Delete("tKasDetil", ObjDb.Where);
        //                }
        //            }
        //        }
        ...
        protected void grdMemoJurnalD_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                hdnId.Value = noPO;
                DataSet MySet = ObjDb.GetRows("select distinct kdTran, tgl, jenisTran,b.namacabang,a.nomatauang,a.kurs from tKasDetil a inner join mcabang b on a.nocabang=b.nocabang where kdTran = '" + noPO + "'");
                if (MySet.Tables[0].Rows.Count > 0)
                {
                    ...
                    DataSet MySetX = ObjDb.GetRows("select distinct sum(debet) as totalDebet,sum(Kredit) as totalKredit from tKasDetil a where kdTran = '" + noPO + "'");

                    if (MySetX.Tables[0].Rows.Count > 0)
                    {
                        decimal totaldebit = 0;
                        decimal totalKredit = 0;
                        DataRow MyRowX = MySetX.Tables[0].Rows[0];
                        totaldebit = Convert.ToDecimal(MyRowX["totalDebet"].ToString());
                        totalKredit = Convert.ToDecimal(MyRowX["totalKredit"].ToString());
                        txtDebitTotal.Text = ObjSys.IsFormatNumber(Convert.ToDecimal(totaldebit).ToString());
                        txtKreditTotal.Text = ObjSys.IsFormatNumber(Convert.ToDecimal(totalKredit).ToString());
                    }
                    SetInitialRow(noPO);
                    LoadDisable();
                    CloseMessage();
                ...
                try
                {
                    ...
                    for (int i = 0; i < grdMemoJurnal.Rows.Count; i++)
                    {
                        ...
                        if (txtAccount.Text != "")
                        {
                            ...
                            ObjGlobal.Param.Add("nocabang", cbocabangunit.Text);

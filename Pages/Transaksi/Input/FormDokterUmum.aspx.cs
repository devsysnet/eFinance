...
namespace eFinance.Pages.Transaksi.Input
{
    ...
    public partial class FormDokterUmum : System.Web.UI.Page
    {
        ...
        private void SetInitialRow()
        {
            ...
            dr["hdnKode"] = string.Empty;
            dr["lblDescription"] = string.Empty;
            dr["lblStok"] = string.Empty;
            dr["txtQty"] = string.Empty;
            dr["lblSatuan"] = string.Empty;
            dr["txtHrg"] = string.Empty;
            dr["txtKet"] = string.Empty;
            dr["hdnIndexSA"] = string.Empty;
        ...
        protected void loadAwal()
        {
            ...
            grdRegister.DataSource = ObjGlobal.GetDataProcedure("SPLoadRegister", ObjGlobal.Param);
        ...
        protected void btnProses_Click(object sender, EventArgs e)
        {
            ...
            if (txtBerat.Text == "")
            {
                message = ObjSys.CreateMessage("Berat Badan harus di isi.");
                valid = false;
            }
            if (txtShBadan.Text == "")
            {
                message = ObjSys.CreateMessage("Suhu Badan harus di isi.");
                valid = false;
            }
            if (txttekanan.Text == "")
            {
                message = ObjSys.CreateMessage("Tekanan Darah harus di isi.");
                valid = false;
            }
            if (txtAnamnesa.Text == "")
            {
                message = ObjSys.CreateMessage("Anamnesa harus di isi.");
                valid = false;
            }
            if (txtDiagnosa.Text == "")
            {
                message = ObjSys.CreateMessage("Diagnosa harus di isi.");
                valid = false;
            }
            if (txtTindakan.Text == "")
            {
                message = ObjSys.CreateMessage("Tindakan / Terapi harus di isi.");
                valid = false;
            }
            if (txtBiayaObat.Text == "")
            {
                message = ObjSys.CreateMessage("Biaya Obat harus di isi.");
                valid = false;
            }
            ...
            try
            {
                ...
                if (valid == true && cekData > 0)
                {
                    ...
                    ObjDb.Data.Add("beratbadan", txtBerat.Text);
                    ObjDb.Data.Add("tinggibadan", txtTinggi.Text);
                    ObjDb.Data.Add("tekanandarah", txttekanan.Text);
                    ObjDb.Data.Add("suhubadan", txtShBadan.Text);
                    ObjDb.Data.Add("anamnesa", txtAnamnesa.Text);
                    ObjDb.Data.Add("diagnosa", txtDiagnosa.Text);
                    ObjDb.Data.Add("tindakan", txtTindakan.Text);
                    ...
                    for (int i = 0; i < grdObat.Rows.Count; i++)
                    {
                        ...
                        if (hdnKode.Value != "" && hdnIndexSA.Value != "")
                        {
                            ...
                            ObjDb.Data.Add("qtySA", Convert.ToDecimal(Convert.ToDecimal(lblStok.Text) - Convert.ToDecimal(txtQty.Text)).ToString());
                            //ObjDb.Data.Add("sisaSA", Convert.ToDecimal(Convert.ToDecimal(lblStok.Text) - Convert.ToDecimal(txtQty.Text)).ToString());

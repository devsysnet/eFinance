...
{
    public partial class TransApproveBudget : System.Web.UI.Page
    {
        ...
        protected void loaddatacombo(string tahun = "")
        {
            ...
            string lvlappr = "";
            string jml = "0";
            DataSet mySety = ObjDb.GetRows("select count(*) as jml from mstApprove where nouser = '" + ObjSys.GetUserId + "' and noParameterApprove = 2 and peruntukan = 'Unit' ");
            ...
            DataSet mySetxx = ObjDb.GetRows("select distinct levelApprove from mstApprove where nouser = '" + ObjSys.GetUserId + "' and noParameterApprove = 2 and peruntukan = 'Perwakilan' ");
            DataRow myRowy = mySety.Tables[0].Rows[0];
             
            if (myRowy["jml"].ToString() == "0")
            {
                DataRow myRowxx = mySetxx.Tables[0].Rows[0];
                lvlappr = myRowxx["levelApprove"].ToString();
            }
            else
            {
        ...
        protected void loaddata(string tahun = "", string cabang = "")
        {
            ...
            ObjGlobal.Param.Add("tahun", tahun);
            ObjGlobal.Param.Add("noParameter", "2");
            GridView2.DataSource = ObjGlobal.GetDataProcedure("SPLoadDataApproveBudget", ObjGlobal.Param);
            GridView2.DataBind();

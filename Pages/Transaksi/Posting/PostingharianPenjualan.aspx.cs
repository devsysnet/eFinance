...
{
    public partial class PostingharianPenjualan : System.Web.UI.Page
    {
        ...
        protected void loadData()
        {
            ...
            grdHarianGL.DataSource = ObjGlobal.GetDataProcedure("SPLoadDataPostHarianPenjualan", ObjGlobal.Param);
        ...
        {
            string Kode = "";
            Kode = ObjSys.GetCodeAutoNumberNew("1", Convert.ToDateTime(dtMulai.Text).ToString("yyyy-MM-dd"));
            ...
            ObjGlobal.ExecuteProcedure("SPpsotingharianPenjualan", ObjGlobal.Param);

            ObjSys.UpdateAutoNumberCodeNew("1", Convert.ToDateTime(dtMulai.Text).ToString("yyyy-MM-dd"));
        ...
        protected void cboAccount_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                DataSet myDate = ObjDb.GetRows("select a.norek,b.kdrek,min(tgljual) as tglMulai from TranspenjualanObat_H a inner join mrekening b on a.norek=b.norek where a.sts=0 and a.norek = '" + cboAccount.SelectedValue + "' and a.noCabang = '" + ObjSys.GetCabangId + "' group by a.norek,b.kdrek");
